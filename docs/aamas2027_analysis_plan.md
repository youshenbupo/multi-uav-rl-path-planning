...
| 8-UAV full vs independent no-uncertainty | Full method: `outputs/core_8uav_post_actor_isolation_evaluations_20260801_uncertainty_predictive_graph/` has five eligible independently trained checkpoints by six scenarios by 20 episodes, complete identity-audited raw JSONL and all-source replay provenance. The independent post-isolation no-uncertainty arm now has five valid training roots, seeds `20260719`--`20260723`, under `outputs/core_8uav_post_actor_isolation_ablation/`; seed `20260722` uses the valid `retry2` root after two launcher-only/interrupted attempts. Its fresh 30-cell evaluation matrix and 35-input all-source replay are still required. Seeds `20260720`/`20260721`/`20260722`/`20260723` retain 457/446/429/457 fully accounted sources and zero-error eligible per-seed replays; these remain diagnostic provenance only. Historical `outputs/core_8uav_evaluations/` and `outputs/core_8uav_ablation_evaluations_20260729/` are retained only. | None until the no-uncertainty arm is fully evaluated/replayed and a fresh procedure is specified. Historical `outputs/paired_summaries/8uav_full_vs_no_uncertainty_20260729.json` is excluded. | The preflight identity-render typo remains a no-result attempt; all historical training/evaluated cells are additionally protocol-ineligible. Seed-20260721's first 450-record replay duplicates four compatibility-alias events and is preserved/excluded; only `aliasdedup_rerun1` is eligible. Seed-20260722's zero-child `Start-Process` failure and stderr-induced interrupted `retry1` root are preserved/excluded; only `retry2` is eligible. All-source versus separate per-seed replay status differences are retained as diagnostic timing/order sensitivity, not selectively reported. |
...
The final seven post-isolation files above are eligible diagnostic provenance;
