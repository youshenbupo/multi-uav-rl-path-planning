...
| --- | --- | --- | --- |
| 3-UAV four arms | Post-isolation roots: MLP `outputs/core_3uav_post_actor_isolation_evaluations/`; raw graph `outputs/core_3uav_post_actor_isolation_evaluations_20260731_raw_graph_mappo_rerun1/`; predictive no-uncertainty `outputs/core_3uav_post_actor_isolation_evaluations_20260801_predictive_graph/`; uncertainty-aware `outputs/core_3uav_post_actor_isolation_evaluations_20260801_uncertainty_predictive_graph/`. Each has five eligible checkpoints by six scenarios by 20 episodes. | `outputs/paired_summaries/post_actor_isolation_3uav_four_arm_descriptive_20260801.json`; independently recomputed and validated in `docs/post_actor_isolation_descriptive_validation.md`. | Historical `outputs/core_3uav_evaluations/` and its paired summary remain protocol-ineligible. Preserve/exclude all documented interrupted/prelaunch roots, including MLP telemetry retry, raw/predictive failed roots, and the two uncertainty seed-20260719 interruptions. |
| 5-UAV full vs independent no-uncertainty | Full method: `outputs/core_5uav_post_actor_isolation_evaluations_20260801_uncertainty_predictive_graph/`; independent no-uncertainty: `outputs/core_5uav_post_actor_isolation_evaluations_20260801_predictive_graph/`. Each has five eligible independently trained checkpoints by six scenarios by 20 episodes, complete identity-audited raw JSONL and all-source replay provenance. | `outputs/paired_summaries/post_actor_isolation_5uav_full_vs_no_uncertainty_descriptive_20260801.json`; delta is full minus no uncertainty and is independently validated. Historical paired JSON remains excluded. | Historical `outputs/core_5uav_evaluations/`, `outputs/core_5uav_ablation_evaluations_rerun4/`, initial/rerun1/rerun2/interrupted-rerun3 roots, the post-isolation non-retry full-method seed-20260721 root/checkpoint, and the zero-artifact seed-20260720 one-second prelaunch-timeout sidecar remain excluded. Seed-20260723's PowerShell stderr-redirection host code is a reproduced launcher-envelope artifact, not a failed training root. |
| 8-UAV full vs independent no-uncertainty | Full method: `outputs/core_8uav_post_actor_isolation_evaluations_20260801_uncertainty_predictive_graph/`; independent no-uncertainty: `outputs/core_8uav_post_actor_isolation_evaluations_20260801_predictive_graph/`. Each now has five eligible independently trained checkpoints by six scenarios by 20 episodes, complete identity-audited raw JSONL and 35-input all-source replay provenance. Seed `20260722` in the no-uncertainty arm uses the valid `retry2` root after two launcher-only/interrupted attempts. Its matrix has 30 cells / 600 records and zero evaluation fallback events; the all-source replay retains 1,790 training events with zero errors. Historical `outputs/core_8uav_evaluations/` and `outputs/core_8uav_ablation_evaluations_20260729/` are retained only. | `outputs/paired_summaries/post_actor_isolation_8uav_full_vs_no_uncertainty_descriptive_20260801.json`; delta is full minus no uncertainty and is independently validated. Historical paired JSON remains excluded. | The preflight identity-render typo remains a no-result attempt; all historical training/evaluated cells are additionally protocol-ineligible. Seed-20260721's first 450-record replay duplicates four compatibility-alias events and is preserved/excluded; only `aliasdedup_rerun1` is eligible. Seed-20260722's zero-child `Start-Process` failure and stderr-induced interrupted `retry1` root are preserved/excluded; only `retry2` is eligible. The no-uncertainty all-source replay has 1,330 fallbacks versus 1,285 summed from separate per-seed replays; preserve this timing/order sensitivity and do not select between them. |
...
The final eight post-isolation files above are eligible diagnostic provenance;
