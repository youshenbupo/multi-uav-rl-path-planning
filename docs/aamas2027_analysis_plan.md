...
| 5-UAV full vs independent no-uncertainty | Full method: `outputs/core_5uav_post_actor_isolation_evaluations_20260801_uncertainty_predictive_graph/` (five eligible checkpoints by six scenarios by 20 episodes). The independent post-isolation no-uncertainty arm has two valid training roots, seeds `20260719` and `20260720`, but no eligible checkpoint-evaluation input yet; three further trainings and the complete matrix/replay remain required. | None until the independent post-isolation no-uncertainty arm is fully trained/evaluated and a fresh procedure is specified. Historical paired JSON remains excluded. | Historical `outputs/core_5uav_evaluations/`, `outputs/core_5uav_ablation_evaluations_rerun4/`, initial/rerun1/rerun2/interrupted-rerun3 roots, the post-isolation non-retry full-method seed-20260721 root/checkpoint, and the zero-artifact seed-20260720 one-second prelaunch-timeout sidecar remain excluded. |
...
The final 5-UAV file above is eligible post-isolation diagnostic provenance;
it is not a safety or fallback-rate result. All preceding historical CBF files
in the list remain protocol-ineligible for aggregate reporting after the
